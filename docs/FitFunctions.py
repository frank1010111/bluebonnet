from scipy import average,sqrt,array,arange,zeros,log,exp,meshgrid
...
def obfun(params,Days,Gas,pvt_gas,PressureTime):
    ...
    res_realgasM.simulate(t,PressureTime=PressureTime)  
    rf2M = res_realgasM.recovery_factor()
    print("Simulation took {:5.2g} s; tau is {:7.5g}, Pi is {:7.5g} and M is {:7.5g}".format(time()-t0,tau,Pi,M))
    Model=rf2M
...
def CheckRefrack(Rates,Months):
    Sqrt=1./sqrt(arange(1,Months+1,1))
    Refrack=False
    params=Parameters()
...
def FitData(DataIndex,Times,Cums,ScaledCumulative,plot='Y',FirstMonth=3,tau0=10,M0=.01,MMax=20000,tlimit=240,CI=False,t0=.5,API='',method='Nelder',PlotFileName='test1.pdf',ylabel="Production (MMcf)"):
    if API =='':
        k=list(Times.keys())
    ...
    #t0=0
    dt0=0
    mini=Minimizer(obfun,params,fcn_args=(Times,Cums,DataIndex,ScaledCumulative,t0,FirstMonth))
    ...
    if plot=='Y':
        ...
        plt.xlabel('Square root time $\sqrt{t}$,  time in months')
        plt.ylabel(ylabel)
